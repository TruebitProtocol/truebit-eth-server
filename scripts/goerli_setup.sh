...
# Start Clef and Geth
GETH=$(echo 'nohup geth --rpc --nousb --goerli --syncmode light --signer' $CLEF_IPC)
...
screen -d -m -S ClefSession
screen -S ClefSession -p 0 -X stuff "stty -echo^M"
screen -S ClefSession -p 0 -X stuff "$CLEF_CMD^M"
sleep 0.25
screen -S ClefSession -p 0 -X stuff "ok^M$CLEF_PWD^M"
until [ -S $CLEF_IPC ]; do sleep 0.1; done;
$GETH 2>> ~/logs/geth_session.log &

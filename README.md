...
truebit-eth-server provides a better way to participate in the Truebit network as a solver or verifier, using a turnkey, appliance-like set-up. This repository is primarily intended to disclose the internals of the truebit-eth-server Docker image hosted in Docker Hub and elsewhere. However, it may be useful for those who have specific server applications that need to adjust Truebit behavior to integrate better with those applications.
...
# What is Truebit?
(Note, the following instructions were written for the truebit-eth Docker image, but apply equally to the truebit-eth-server image if you simply substitute the -server name where appropriate in Docker commands.)
...
2. [Running truebit-eth-server As a Server](#Running-truebit-eth-server-as-a-server)
3. [Solve and verify tasks interactively](#Solve-and-verify-tasks-interactively)
4. [Getting data into and out of Truebit](#Getting-data-into-and-out-of-Truebit)
5. [Client configuration](#Client-configuration)
6. [Building your own tasks](#Building-your-own-tasks)
7. [Native installation](#Native-installation)
8. [Contract API reference](#Contract-API-reference)
...
```bash
docker pull truebitprotocol/truebit-eth-server:latest
```
...
docker run --network host -v $YYY/docker-clef:/root/.clef -v $YYY/docker-geth:/root/.ethereum -v $YYY/docker-ipfs:/root/.ipfs --name truebit --rm -it truebitprotocol/truebit-eth-server:latest /bin/bash
```
...
```
If you omitted the `--name truebit` flag when starting your container (i.e., you did not cut and paste the command [above](#Start-container)), you will need to find the name of the container running `truebitprotocol/truebit-eth-server:latest` by using `docker ps`.  Then, in place of "`truebit`" in the `docker exec` incantation above, substitute either your container's name, which might look something like `xenodochial_fermat`, or the container's ID, which looks something like `859841f65999`.
...
# Solve and verify tasks interactively
